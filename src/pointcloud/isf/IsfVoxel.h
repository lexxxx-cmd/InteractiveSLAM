...
/**
 * @brief 逐帧 LOD 的最低**目标**点数：低于此值就不再生成更粗的级别
 *
 * 这是**逐帧**门槛（单帧点数一般在 1 千~2 万），不是整图门槛。原先该常量取 4096
 * 且放在 IsfFormat.h，后果是单帧点数 < 8192 的图**一级 LOD 都生成不出来**——实测
 * 559 帧 / 每帧约 5045 点的地图，.isf 里 L1=L2=L3=0、LOD 阶段耗时 0.0 ms；单帧
 * 约 13,004 点的大图也只能生成 L1（L2 目标 3251 < 4096 即 break）。而渲染侧
 * CloudRenderData 用的是固定 8，同一批数据能生成三级，于是"从 .isf 分页"与
 * "直接构建 chunk"的 LOD 行为对不上。现在两条路径统一到本常量。
 */
constexpr uint32_t kMinLodPoints = 64;
